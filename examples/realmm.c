...
float Mm(int run) {
  ...
  return rmr[run + 1][run + 1];
}

int main() {
  int i;
  float results[] = {
      -776.000000f, 636.888889f,  961.777778f, -239.555556f, -337.777778f,
      -298.222222f, -962.666667f, 94.666667f,  62.222222f,   33.333333f,
  };
  for (i = 0; i < 10; i++) {
    assert(fabs(Mm(i) - results[i]) < 0.001f);
  }
  return 0;
}
