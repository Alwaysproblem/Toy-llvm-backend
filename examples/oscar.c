...
void Printcomplex(struct complex zarray[], int start, int finish,
  ...
  i = start;
  float results[] = {-9.365, -3.987, -7.705, -3.928, 1.768,  5.818,  4.053,
                     0.251,  4.150,  -5.627, 7.061,  3.181,  -2.217, 1.677,
                     1.318,  4.861,  2.666,  7.732,  6.826,  5.291,  -1.201,
                     -7.463, 3.584,  4.470,  6.182,  -3.909, -8.408, 2.400,
                     4.814,  -1.604, -9.150, -0.920};
  int j = 0;
  do {
    assert(fabs(zarray[i].rp - results[j++]) < 0.01);
    assert(fabs(zarray[i].ip - results[j++]) < 0.01);
    i = i + increment;
    assert(fabs(zarray[i].rp - results[j++]) < 0.01);
    assert(fabs(zarray[i].ip - results[j++]) < 0.01);
    i = i + increment;
